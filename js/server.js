...
var GameState = (function () {
  ...
  GameState.prototype.getTeam = function () {
    return _TEAM
  }
  GameState.prototype.getRound = function () {
    return _ROUND
  }
  GameState.prototype.getPoints = function () {
    return _POINTS
  }
  GameState.prototype.getGames = function () {
    return _GAMES
  }
  GameState.prototype.getRebusQuestions = function () {
    return _REBUS_QUESTIONS
  }
  GameState.prototype.getRebusIndex = function () {
    return _REBUS_INDEX
  }
  GameState.prototype.getDoublePointsBid = function () {
    return _BID
  }
  GameState.prototype.getWagers = function () {
    return _WAGERS
  }

  GameState.prototype.setTeam = function (team) {
    _TEAM = team
    localStorage.team = JSON.stringify(_TEAM)
  }
  GameState.prototype.setRound = function (round) {
    _ROUND = round
    localStorage.round = JSON.stringify(_ROUND)
  }
  GameState.prototype.setPoints = function (points) {
    _POINTS = points
    localStorage.points = JSON.stringify(_POINTS)
  }
  GameState.prototype.setGames = function (games) {
    _GAMES = games
    localStorage.games = JSON.stringify(_GAMES)
  }
  GameState.prototype.setRebusQuestions = function (questions) {
    _REBUS_QUESTIONS = questions
    localStorage.rebusQuestions = JSON.stringify(_REBUS_QUESTIONS)
  }
  GameState.prototype.setRebusIndex = function (index) {
    _REBUS_INDEX = index
    localStorage.rebusIndex = JSON.stringify(_REBUS_INDEX)
  }
  GameState.prototype.setDoublePointsBid = function (bid) {
    _BID = bid
    localStorage.bid = JSON.stringify(_BID)
  }
  GameState.prototype.setWagers = function (wagers) {
    _WAGERS = wagers
    localStorage.wagers = JSON.stringify(_WAGERS)
  }

  GameState.prototype.hideBlock = function () {
    _ACTIVE_GAME.blocks[_ACTIVE_GAME.active_cell.column][
      _ACTIVE_GAME.active_cell.row
    ].hidden = true
  ...
  GameState.prototype.getNewQuestion = function (block, categoryIndex) {
    ...
    if (questions[i] === undefined) {
      alert('Error: Ran out of questions?')
    }

    _ACTIVE_GAME.active_question = $.extend(true, {}, questions[i])
    _ACTIVE_GAME.active_cell = {
...
function runGame(index) {
  ...
  if (game.type == 'rebus') {
    ...
    if ('scoring' in game) sendCommand('showScores', game.scoring)
    else sendCommand('showScores', true)
    sendCommand('sizeStaticElements')
    $('#timeout').show()
...
var clientScreen = window.open(
  'client.html',
  getWinName('client.html'),
  'toolbar=0,location=0,menubar=0'
)
if (!clientScreen)
  alert(
    'ERROR: Pop-up blocker seems to be enabled. Please allow popups for the client'
  )
...
function initialize() {
  $.each(GAMES, function (i, v) {
    if (v.type == 'rebus') {
      ...
      v.blocks = Array(v.board.length).fill(
        Array(v.pointValues.length).fill({})
      )
      v.indices = Array(v.board.length).fill(0)
      v.double_points = -1
      if (v.scoring !== false) {
        v.double_points = Math.floor(
          Math.random() * (v.board.length * v.pointValues.length)
        )
      }
      v.index = 0
    }
  ...
  $.each(CONFIG.teams, function (i, v) {
    $('#scores_names').append(
      '<td width="' + 100 / CONFIG.teams.length + '%">' + v + '</td>'
    )
    $('#scores_values').append(
      '<td><input type="number" id="overridePoints-' +
        i +
        '" data-team="' +
        i +
        '" class="overridePoints" /></td>'
    )
    $('#scores_wagers').append(
      '<td><input type="number" id="wagerPoints-' +
        i +
        '" data-team="' +
        i +
        '" class="wagerPoints" placeholder="wager" /></td>'
    )
    $('#scores_actions').append(
      '<td> \
        <button data-team="' +
        i +
        '" data-award="true" class="updateWager wagerSuccess">&#x2714;</button> \
        <button data-team="' +
        i +
        '" data-award="false" class="updateWager wagerFail">&#x2716;</button> \
    </td>'
    )
  })
...
function correct() {
  ...
  if (game.type == 'rebus') {
    ...
    if (isNaN(pts)) {
      console.log('Skipping non-point question')
      return
    }
...
function renderAnswer(answer) {
  ...
  timer = setInterval(function () {
    if ($('#timeout-input').val() <= 0) {
      clearInterval(timer)
      return
    }
    $('#timeout-input')
      .val($('#timeout-input').val() - 1)
      .trigger('change')
  }, 1000)
}
...
function updateDoubleCounter(game) {
  ...
  if (!d || d <= 0) {
    ...
    $('#doubleCounter')
      .html('Double Points in <strong>' + d + '</strong> questions')
      .css('display', 'inline')
  } else {
    $('#doubleCounter')
      .html('Double Points <strong>next question</strong>')
      .css('display', 'inline')
  }
}
...
$(function () {
  ...
  $(window).keydown(function (e) {
    ...
    if (team >= 0 && team < CONFIG.teams.length) {
      gameState.setTeam(team)
      sendCommand('highlightTeam', gameState.getTeam())
      if (gameState.getActiveCell() !== undefined) {
        CAN_SCORE = true
      }
    }
  ...
  $('#categories').on('click', '.categoriesOption', function (e) {
    ...
    if (
      ...
      typeof gameState.getActiveCell().points == 'number'
    ) {
  ...
  $('#rounds').on('change', function (e) {
    ...
    runGame(i)
    sendCommand('highlightTeam', gameState.getTeam())
    if (gameState.getGame(i + 1) === undefined) {
      $('#next-round').hide()
    }
  })
  ...
  $('#destroyState').on('click', function (e) {
    if (
      confirm(
        'Are you sure you want to delete the existing game state?  This will reload the game.'
      )
    ) {
      localStorage.clear()
      location.reload()
  ...
  $('#scoreModal').on('click', '.updateWager', function (e) {
    ...
    if (!succeeded) {
      ...
      $('#overridePoints-' + team)
        .addClass('wagerFail')
        .removeClass('wagerSuccess')
    } else {
      $('#overridePoints-' + team)
        .addClass('wagerSuccess')
        .removeClass('wagerFail')
    }
  ...
  $('#revealTrivia').on('click', function (e) {
    sendCommand('revealFinalTrivia')
  })
  $('#revealLightning').on('click', function (e) {
    sendCommand('revealLightning')
  })
