...
function showAudience(visible) {
  if (visible)
    $('#audience')
      ...
      .animate({ opacity: 1 }, CONFIG.transitionDuration)
...
function buildRebus(game) {
  ...
  if (game.active_question !== undefined) {
    ...
    if ($('#answerDisplay').length > 0) {
      renderAnswer(game.active_question.answer)
...
function clearCategoriesBlock(arr) {
  $('#categoriesColumn-' + arr[0] + ' .categoriesOption')
    .eq(arr[1])
    .addClass('clear')
}
...
function doublePoints(show) {
  if (show) $('#doublePoints').css('display', 'table').dynasize()
  else $('#doublePoints').hide()
}
...
function showIntermission(game) {
  ...
  if (/\.((png)|(jpg)|(jpeg)|(gif)|(svg))$/i.test(game.source)) {
    $('#intermission').css({
      'background-image': 'url(' + game.source + ')',
      display: 'none'
    })
  } else if (/\.((swf)|(flv))$/i.test(game.source)) {
    $('#intermission')
      .css({ display: 'none' })
      .html('<embed src="' + game.source + '">')
  } else if (/\.((mov)|(mp4))$/i.test(game.source)) {
    $('#intermission')
      .css({ display: 'none' })
      .html(
        '<video autoplay' +
          (game.loop ? ' loop muted' : '') +
          ' src="' +
          game.source +
          '">'
      )
  ...
  if (game.text) {
    ...
    $('#intermission')
      .append(
        '<span style="position: absolute; z-index: 2; top: 0;">' +
          game.text +
          '</span>'
      )
      .dynasize()
...
function buildFinalTrivia(game) {
  ...
  // Update controls
  $('#controls button').not('#openMenu').hide()
  $('#startFinalTimer, #revealTrivia, #revealLightning')
    .show()
    .prop('disabled', false)
...
function revealFinalTrivia() {
  ...
  $('#finalTriviaQuestion')
    .show()
    .velocity('transition.slideUpIn', { display: 'table' })
  $('#solution').show()
...
function revealLightning() {
  ...
  $('#finalTriviaTitle, #finalTriviaQuestion').velocity(
    'transition.slideUpOut',
    {
      ...
      complete: function () {
        ...
        sizeStaticElements()
        $('#lightningTitle, #lightningQuestion').velocity(
